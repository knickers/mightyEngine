...
tooth::tooth() {
	double mA[] = {0,57,60,25,-25,-60,-57,0};
	point2 mP[] = { {-1.5,1.5},{-1.2,1.2},{1.0,0.7},{1.5,0.3},
					{1.5,-0.3},{1.0,-0.7},{-1.2,-1.2},{-1.5,-1.5}};
	mWidth = 1;
}
void tooth::SetWidth(double w) {
	mWidth = w;
}
...
MYT::MYT() {
	...
	SetNumVanes(mVanes);
	mArcLength = mTooth.GetArcLength();
}
...
//	mSmallGear = 2*PI*mSmallGear / mArcLength;
...
void tooth::DrawFace() {
	glBegin(GL_POLYGON);
	for (int i=0; i<8; i++) {
		glVertex2d(mP[i].x, mP[i].y);
	}
	glEnd();
}

void MYT::DrawToothFace() {
	glBegin(GL_POLYGON);
	glVertex2d(-1.5,1.5);
	glVertex2d(-1.2,1.2);
	glVertex2d(1,.7);
...
void tooth::DrawEdge(int i, int j, float nx, float ny) {
	glBegin(GL_QUADS);
	glVertex3d(mP[i].x, mP[i].y, mWidth);
	glVertex3d(mP[i].x, mP[i].y,-mWidth);
	glNormal3f(nx, ny, 0);
	glVertex3d(mP[j].x, mP[j].y,-mWidth);
	glVertex3d(mP[j].x, mP[j].y, mWidth);
	glEnd();
}

void MYT::DrawToothEdge(double x1,double y1,double x2,double y2,float nx,float ny) {
	glBegin(GL_QUADS);
...
void tooth::DrawEdges() {
	//   4_
	//  3/ \5
	//  2| |6
	// 1/___\7
	for (int i=0; i<8; i++) {
		glNormal3d(Cos(mA[i]), Sin(mA[i]), 0);
		DrawEdge(i,i+1, Cos(mA[i+1]),Sin(mA[i+1]));
	}
}

void MYT::DrawToothEdges() {
	//   4_
	//  3/ \5
	//  2| |6
	// 1/___\7
	glNormal3d(Cos(0), Sin(0), 0);
	DrawToothEdge(-1.5, +1.5, -1.2, +1.2, Cos(+57), Sin(+57)); // 1 - 45 deg
	glNormal3d(Cos(+57), Sin(+57), 0);
	DrawToothEdge(-1.2, +1.2, +1.0, +0.7, Cos(+60), Sin(+60)); // 2 - 70 deg
	glNormal3d(Cos(+60), Sin(+60), 0);
	DrawToothEdge(+1.0, +0.7, +1.5, +0.3, Cos(+25), Sin(+25)); // 3 - 50 deg
	glNormal3d(Cos(+25), Sin(+25), 0);
	DrawToothEdge(+1.5, +0.3, +1.5, -0.3, Cos(-25), Sin(-25)); // 4 - 00 deg
	glNormal3d(Cos(-25), Sin(-25), 0);
	DrawToothEdge(+1.5, -0.3, +1.0, -0.7, Cos(-60), Sin(-60)); // 5 - 50 deg
	glNormal3d(Cos(-60), Sin(-60), 0);
	DrawToothEdge(-1.2, -1.2, +1.0, -0.7, Cos(-57), Sin(-57)); // 6 - 70 deg
	glNormal3d(Cos(-57), Sin(-57), 0);
	DrawToothEdge(-1.2, -1.2, -1.5, -1.5, Cos(0), Sin(0)); // 7 - 45 deg
}

void tooth::Draw() {
	// Bottom
	glNormal3d(0,0,-1);
	glTranslated(0,0,-mWidth);
		DrawFace();
	glTranslated(0,0, mWidth);
	// Top
	glNormal3d(0,0,1);
...
void MYT::DrawGear(double outRadius, double inRadius, int teeth) {
	//teeth = (2*PI*outRadius) / mArcLength;
//	printf("Calculated teeth: %d r=%f\n", 2*PI*outRadius/mArcLength, outRadius);
	double angle = 360.0/(double)teeth;
	double rad = outRadius - inRadius;
	// Body
	glTranslated(0,0,-mGearWidth/2);
		DrawCurveBox(rad, mGearWidth, rad/2, 360, teeth, 0);
	glTranslated(0,0,-mGearWidth/2);
	// Bearing Collar
	DrawCurveBox(rad/4, mGearWidth*2, inRadius+rad/8, 360, teeth, 0);
	// Tooth Collar
	DrawCurveBox(rad/4, mGearWidth*2, outRadius-rad/8-1.4, 360, teeth*2, 0);
	glTranslated(0, 0, mGearWidth);
	// Teeth
...
void MYT::Move(double speed) {
	...
	if (mFrame == 10) {
		mRPM   = clock() - mTime;
		mRPM   = (speed/mRPM)*(1.0/360)*(1000/1.0)*(1000/1.0)*(60 /1.0)*mFrame;
		// rpm = ( deg / ns )*(rev/deg)*( ns / ms)*( ms /sec)*(sec/min)*frames
		mFrame = 0;
		mTime  = clock();
	} else
		mFrame++;
...
void MYT::Draw() {
	...
	glTranslated(-40,0,0);
		glRotated(-90,0,1,0);
			glRotated(mRotate+90,0,0,1);
				mBlueAngle = DrawHalfSup(1);
				mBlueAngle = -mRotate-mBlueAngle;
	glPopMatrix();
	...
	glTranslated(-40,0,0);
		glRotated(-90,0,1,0);
			glRotated(mRotate+270,0,0,1);
				mTanAngle = DrawHalfSup(0);
				mTanAngle = -mRotate-mTanAngle;
	glPopMatrix();
	// torus cylinder
	glPushMatrix();
	...
	glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, ivery);
	glPushMatrix();
		glRotated(mTanAngle + mTanOffset,1,0,0);
			glTranslated(60,0,0);
				glCallList(ROTOR);
	glPopMatrix();
	glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, blue);
	glPushMatrix();
		glRotated(mBlueAngle + mBlueOffset,1,0,0);
			glTranslated(60,0,0);
				glRotated(180,0,0,1);
					glCallList(ROTOR);
	glPopMatrix();
	...
	if (mShowStages) {
		glPushMatrix();
			glRotated(mTanAngle,1,0,0);
				glTranslated(60,0,0);
					glRotated(180,0,0,1);
						DrawStages(mBlueAngle, mTanAngle);
		glPopMatrix();
	}
	glPopMatrix();
	// ports
	double wid = (10.0-mVanes)*2 + 7;
...
void MYT::DrawMenu() { // Menu to display instructions
	...
	// Text
	glColor3fv(white);
	text_output(L+2.25*P,  T-3*P, (char*)"- Press 'tab' to toggle this menu.");
	text_output(L+2.25*P,  T-5*P, (char*)"- Use the arrow keys or click and drag to rotate the model.");
	text_output(L+2.25*P,  T-7*P, (char*)"- Drag the 'speed' and 'zoom' slider bars up and down");
	text_output(L+2.25*P,  T-9*P, (char*)"- Choose the number of vanes with the number keys.");
	text_output(L+2.25*P, T-11*P, (char*)"");
}
